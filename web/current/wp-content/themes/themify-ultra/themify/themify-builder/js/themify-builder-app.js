...
(function($){
			...
			var callback = function (response) {
				...
				for (iterate = 0; iterate < inputs.length; ++iterate) {
							...
							$this_option.find('.themify_builder_row').each(function (r) {
								$(this).find('.tfb_lb_option_child').each(function (i) {
									...
									if ($this_option_child.hasClass('tfb_lb_wp_editor') && !$this_option_child.hasClass('clone')) {
										api.Utils.initQuickTags(this_option_id_real);
										if (typeof tinyMCE !== 'undefined') {
											api.Utils.initNewEditor(this_option_id_real);
										}
									}
					...
					if ($this_option.hasClass('tfb_lb_wp_editor')) {
						...
						if (typeof tinyMCE !== 'undefined') {
							api.Utils.initNewEditor(this_option_id);
						}
					}
	...
	api.Views.SubRows['default'] = api.Views.BaseElement.extend({
		...
		renderInlineData: function() {
			...
			if ( 'equal-column-height' === this.model.get('equal_column_height') && ! this.$('.themify_builder_equal_column_height_checkbox').is(':checked') ) {
				setTimeout(function(){
					this.$('.themify_builder_equal_column_height_checkbox').prop('checked', true).trigger('change');
				}.bind(this), 400);
			}
		},
		delete: function( event ) {
	...
	api.Views.Rows['default'] = api.Views.BaseElement.extend({
		...
		events: {
			...
			'change .themify_builder_equal_column_height_checkbox' : '_equalColumnHeightChanged',
		...
		_gridMenuClicked: function (event) {
			...
			$.each(set, function (i, v) {
				if ($base.children('.themify_builder_col').eq(i).length > 0) {
					$base.children('.themify_builder_col').eq(i).removeClass(api.Utils.clearClass).addClass('col' + v);
				} else {
					// Add column
					api.Utils._addNewColumn({
						newclass: 'col' + v, 
		...
		_equalColumnHeightChanged: function ( event ) {
			var $this = $(event.currentTarget),
				handle = $this.data('handle');
			if (handle == 'module')
				return;
	...
	api.Mixins.Builder = {
		...
		newRowAvailable: function () {
			var that = this,
				$parent = this.$el.children('.themify_builder_row');
			...
			if ($parent.last().find('.active_module').length > 0 || this.$el.children('.themify_builder_row').length == 0) {
		...
		equalHeight: function () {
			this.makeEqual(this.$el.find('.themify_builder_row:visible'), '.themify_builder_row_content:visible');
			this.makeEqual(this.$el.find('.themify_builder_sub_row:visible'), '.themify_builder_sub_row_content');
		},
	...
	api.Forms = {
		bindEvents: function() {
			...
			// used for both column and sub-column options
				...
				.on('click', '.themify_builder_row_js_wrapper .toggle_row', api.Views.Rows.default.prototype.toggleRow)
				.on('hover', '.themify_builder_row_js_wrapper .row_menu', api.Views.BaseElement.prototype.actionMenuHover);
		},
