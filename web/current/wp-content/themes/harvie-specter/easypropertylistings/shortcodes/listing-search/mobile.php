...
if ( ! empty( $selected_post_types ) ) :
	if ( count( $selected_post_types ) > 1 ):
		...
		$selected_post_types[] = 'development';
	...
	foreach ( $selected_post_types as $post_type ) :
		...
		<form method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			...
			<input type="hidden" name="project_type" id="project_type_mbl" value="" />
			<?php
				...
				foreach ( $epl_frontend_fields as &$epl_frontend_field ) {
					...
					if(in_array($epl_frontend_field['key'],array('search_bed','search_house_category','search_price','search_car','search_bath'))){
						continue;
					}
				...
				<div class="advanced-search-sec" >
					...
					foreach ( $epl_frontend_fields as &$epl_frontend_field ) {
						...
						if(in_array($epl_frontend_field['key'],array('search_bed','search_house_category','search_price','search_car','search_bath'))) {
							...
							if ( $epl_frontend_field['key'] == 'search_bed' && isset( $house_category_multiple ) && $house_category_multiple == 'on' ) {
