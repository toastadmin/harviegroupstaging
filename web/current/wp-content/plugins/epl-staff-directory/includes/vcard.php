...
class EPL_SD_VCARD {
    ...
    public function __construct($author_id)
    {
		...
		$current_author = new EPL_Author_Meta($author_id);
        $this->data = array(
            'display_name' => $current_author->name,
            'first_name' => get_the_author_meta( 'first_name' , $current_author->author_id),
            'last_name' => get_the_author_meta( 'last_name' , $current_author->author_id),
            'additional_name' => NULL,
            'name_prefix' => NULL,
            'name_suffix' => NULL,
            'nickname' => NULL,
            'title' => NULL,
            'role' => $current_author->position,
            'department' => NULL,
            'company' => NULL,
            'work_po_box' => NULL,
            ...
            'home_country' => NULL,
            'office_tel' => $current_author->mobile,
            'home_tel' => NULL,
            'cell_tel' => NULL,
            'fax_tel' => NULL,
            'pager_tel' => NULL,
            'email1' => $current_author->email,
            'email2' => NULL,
            'url' => NULL,
            'photo' => NULL,
            'birthday' => NULL,
...
function initiate_vcard_download() {
	if(isset($_GET['epl_sd_action']) && $_GET['epl_sd_action'] == 'epl_sd_get_vcard' && isset($_GET['author_id']) && intval($_GET['author_id']) > 0 ) {

		$author_id = intval($_GET['author_id']);
		$current_author_vcard = new EPL_SD_VCARD($author_id);
		$current_author_vcard->download();
	}
}
add_action('init','initiate_vcard_download');
